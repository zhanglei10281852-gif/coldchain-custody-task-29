...
func (q *queries) ExpireHandoffs(ctx context.Context, now time.Time, limit int) ([]domain.CustodyHandoff, error) {
	rows, err := q.q.QueryContext(ctx, handoffSelect+` WHERE status = 'pending' AND expires_at < ? ORDER BY expires_at ASC LIMIT ?`, formatTime(now), limit)
	if err != nil {
		return nil, translateError("list expired handoffs", err)
	}
	defer rows.Close()
	handoffs := make([]domain.CustodyHandoff, 0, limit)
	for rows.Next() {
		handoff, err := scanHandoff(rows)
		if err != nil {
			return nil, fmt.Errorf("scan expired handoff: %w", err)
		}
		handoffs = append(handoffs, handoff)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate expired handoffs: %w", err)
	}
	expired := make([]domain.CustodyHandoff, 0, len(handoffs))
	for _, handoff := range handoffs {
		...
		rowsAffected, err := result.RowsAffected()
		if err != nil {
			return nil, fmt.Errorf("expire handoff rows affected: %w", err)
		}
		if rowsAffected == 1 {
